...
void NOMAD::SimplePoll::init()
{
    ...
    // ChT. TODO remove these options that are not as good as basic version
    _refineOnPartial = _runParams->getAttributeValue<bool>("QUAD_MODEL_SEARCH_SIMPLE_MADS_REFINE_MESH_ON_PARTIAL");
    _twoPointsBarrier = _runParams->getAttributeValue<bool>("QUAD_MODEL_SEARCH_SIMPLE_MADS_TWO_POINTS_BARRIER");
    
    // ChT. For now let's forget about fixed variables from the original problem.
    // NEED TO TEST WITH FIXED VARIABLE
...
void NOMAD::SimplePoll::endImp()
{
    ...
    if (refBestFeas.isDefined() || refBestInf.isDefined())
    {
        ...
        // if (success >= SuccessType::FULL_SUCCESS)
        if (success >= trigger)
        {
            OUTPUT_INFO_START
            if (success == NOMAD::SuccessType::PARTIAL_SUCCESS)
