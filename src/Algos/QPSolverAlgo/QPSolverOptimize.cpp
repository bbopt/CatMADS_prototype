...
void NOMAD::QPSolverOptimize::generateTrialPointsImp()
{
    ...
    else
    {
        ...
        const auto mu0 = _runParams->getAttributeValue<Double>("QP_AugLag_mu0").todouble(); // TODO: * std::min(std::sqrt(tol), 1.0);
        ...
        const auto omega0 = _runParams->getAttributeValue<Double>("QP_AugLag_omega0").todouble(); // TODO: * std::min(std::sqrt(tol), 1.0);
...
//        }
        if (SelectAlgo == 0 || SelectAlgo == 1 || SelectAlgo == 2)
        {
            ...
            else if (SelectAlgo == 1)
            {
                ...
                //            // ChT Tmp for testing how frequent we have a some potential difficulties for the solver
                //            if (runOk && status != TRIPMSolverStatus::SOLVED)
                //            {
                //                // Create a switch statement for the status
                //                switch (status)
                //                {
                //                    case TRIPMSolverStatus::MAX_ITER_REACHED:
                //                        std::cout << "Warning: TRIPM solver reached the maximum number of iterations" << std::endl;
                //                        break;
                //                    case TRIPMSolverStatus::LM_FAILURE:
                //                        std::cout << "Warning: TRIPM solver failed with the Levenberg-Marquardt algorithm" << std::endl;
                //                        break;
                //                    case TRIPMSolverStatus::STRICT_PT_FAILURE:
                //                        std::cout << "Warning: TRIPM solver failed with the computation of a strict solution" << std::endl;
                //                        break;
                //                    case TRIPMSolverStatus::TIGHT_VAR_BOUNDS:
                //                        std::cout << "Warning: TRIPM solver failed with the bounds on variables being too tight" << std::endl;
                //                        break;
                //                    case TRIPMSolverStatus::STAGNATION_ITERATES:
                //                        // TOO FREQUENT std::cout << "Warning: TRIPM solver failed with the distance between successive iterates being too low" << std::endl;
                //                        break;
                //                    default:
                //                        break;
                //                }
    ...
    const bool verbose)
{
    ...
    // TODO: one could alo compute the EVD decomposition, which directly gives the eigenvalues
    // and eigenvectors.
...
//     {
//         // Make dk a descent direction and project it into the working set.
//         d_k.multiply(-1.0);
...
//         const int nfreep = n - sum(active_l, n) - sum(active_u, n);
...
//         runOk = ((qm - qmp) <= kappa * diffqmqmp) || (nfree == nfreep);
//         //runOk = (qmp >= qm) || (nfree == nfreep);
...
//     auto active_u = new bool[n];
//     auto binding = new bool[n];
//     auto working = new bool[n]; // subset of active_l OR active_u
//     active_bounds(X, lvar, uvar, active_l, active_u);
//     binding_bounds(Grad, active_l, active_u, binding);
//     for (int i=0; i < n; ++i)
//     {
...
//     {
//         // Generate a sequence of projected gradient steps. Update active_l and active_u at the same time.
//         projectedGradient(X, H, g, g0, lvar, uvar, active_l, active_u, working, Grad, Temp, kappa, atol + ng0 * rtol, n);
//         const double qm = getModelObj(X, H, g, g0);
//         getModelGrad(&Grad, X, H, g); // TODO: projectedGradient return -Grad.

//         // Optimize within the working set
//         // NB: This method does not use projected conjugate gradient
...
//         // Check that it works
//         const double nd = d_k.norm();
//         Xp = X; Xp.add(d_k); // Xp = X + d_k
//         const bool unconstrainedSuccess = (getModelObj(X, H, g) > getModelObj(Xp, H, g)) || (nd <= atol);
//         if (!unconstrainedSuccess)
//         {
//             d_k.fill(0);
//             // verbose && std::cout << getModelObj(X, H, g) << " " << getModelObj(SGTELIB::Matrix::add(X, d_k), H, g)
//             // verbose && std::cout << " " << getModelObj(SGTELIB::Matrix::add(X, SGTELIB::Matrix::product(d_k, 1000.0)), H, g);
...
//             // We diverge here from the More and Toraldo algorithm
//             // Xp = X + d_k a_max
//             Xp = d_k; Xp.multiply(a_max); Xp.add(X);
//             if (getModelObj(Xp, H, g) <= getModelObj(X, H, g))
//             {
...
//         // Update the sets of active and working constraints
...
//         }
//         else if ((!unconstrainedSuccess || a_k == 0) && !update)
//         {
//             OK = true; // no progress
//         }
//         k++;
//         verbose && std::cout << "  k=" << k << " (" << unconstrainedSuccess << ") " << " f(xk)=" << fk << " |d|=" << nd << " a(amax)= " << a_k << " ( " << a_max << " )";
    ...
    const double t_max)
{
    ...
    while (!armijoCond && tk > t_small) // TODO: add this stopping condition: && (nbA < bA_max))
    {
        tk /= t_decrease;
    ...
    const bool verbose)
{
    ...
    // NB: TODO a scaling could be applied to prevent this behavior. But at extremely low tolerance, this is not
...
//     // Compute the set of active, strictly feasible and infeasible constraints
//     SGTELIB::Matrix cons("cons", _nbCons, 1);
//     getModelCons(&cons, X_k);
...
//     // Compute pseudo-gradient of the penalized augmented lagrangian:
//     // grad p = grad f - 1/ mu sum_{active constraints} grad c(x),
//     // where mu = 1.
//     // Note that at initialization, the Lagrange multipliers of the penalized augmented Lagrangian are set at 0,
//     // which explains the formula of the pseudo gradient above.
...
//     // The active multipliers are computed as a least-square solution of
//     // Ak lambda = pseudoGradient_k, where Ak is the jacobian matrix of active
//     // constraints.
//     SGTELIB::Matrix active_multiplier_k("active_multiplier_k", nbActive, 1); // TODO: bad...
//     active_multiplier_k.fill(0.0);
//     active_multiplier_k = SGTELIB::Surrogate_PRS::compute_multiplier(pseudoGradient_k, activeJacobian_k);

//     // Compute || pseudoGradient_k - activeJacobian_k t active_multiplier_k ||.
//     // At optimality, we should have pseudoGradient_k (approx) = activeJacobian_k t active_multiplier_k
//     // which can be used as a stopping criterion.
//     double dual_norm = 0;//compute_dual_residual(pseudoGradient_k, activeJacobian_k, active_multiplier_k);

//     // bool innerSuccess = (dual_norm < tol) && isFeasible(cons, tol); // TODO Not used
...
//         // Compute iterates and sets of constraints for the inner loop
//         getModelCons(&cons, X_k);
//         Jacobian_k = getModelJacobian(X_k);
//         getModelActiveCons(cons, innerPrecision, active);
//         getModelFeasibleCons(cons, innerPrecision, feasible);
...
//             // Compute a horizontal step (minimize the change in the penalty function subject to active
//             // constraints)
//             const bool horizontalSuccess = compute_horizontal_step(X_k, h_k, Jacobian_k, active, feasible, infeasible, lambda_l, mu_l);
//             update(X_can, h_k); // X_can := X_k + h_k
...
//                 // Increase feasibility by computing a vertical step. It is computed using active constraint
//                 // gradients at X_k (and NOT X_k + h_k).
//                 const bool verticalSuccess = compute_vertical_step(X_k, v_k, activeJacobian_k, cons, active);
//                 update(X_can, v_k); // X_can := X_k + h_k + v_k

//                 getModelCons(&cons, X_can);
//                 F_k = getPenalizedL1AugLagModelObj(X_can, cons, lambda_l, mu_l);
//                 std::cout << " V: (l=" << iterInnerLoop << ") |v|= " << v_k.norm() << " Pk=" << F_k << " ||c||= " << cons.norm() << std::endl;
//                 const bool decreasePenalty = F_k < F_km1; // =...
//                 if (decreasePenalty)
//                 {
//                     // Accept the candidate
//                     X_k = X_can;
...
//                     // Recompute a horizontal step
//                     compute_horizontal_step(X_k, h_k, Jacobian_k, active, feasible, infeasible, lambda_l, mu_l);
//                     const double beta_k = piecewise_line_search(X_k, h_k, active , feasible, infeasible, lambda_l, mu_l);
//                     update(X_k, h_k, beta_k);
//                 }
...
//             // Check for failure:
//             const double deltaF = fabs(F_k-F_km1);
//             const double distXkXkm1 = NOMAD::Point::dist(X_k,X_km1).todouble();
//             // normGradLag_k = gradLag_k.norm();
//             F_k = getPenalizedL1AugLagModelObj(X_k, cons, lambda_l, mu_l);
//             std::cout << _nbCons << " " << _n << " " << _m << std::endl;
...
//         // Update Lagrange multipliers and sets of active, infeasible and feasible constraints
//         if (isFeasible(cons, eta_l))
//         { // no update of mu_l
//             for (int i = 0; i < _nbCons ; i++)
//             {
...
//                     feasible[i] = false;
//                     infeasible[i] = false;
//                     lambda_l.set(i, 0, li + min(li, 0).todouble()); // TODO: a lambda_l is missing
//                 }
//                 else if (cons.get(i, 0) < -tol)
//                 {
//                     active[i] = false;
...
//         iterOuterLoop++; // l = l + 1;
//         fk = getModelObj(X_k);
//         std::cout << "k= " << iterOuterLoop <<  " |Proj(x - grad) - x|= " << ngproj  << " f(x)= " << fk << " |c(x)|=" << cons.norm() << std::endl;

//         // Check failure
//         distXOuterLoop = NOMAD::Point::dist(X_k, X_om1).todouble();
//         outerFailure = !unbounded_subpb && (distXOuterLoop <= tolerance_distDX);
//         // outerFailure = outerFailure || innerFailure;
//         outerFailure = outerFailure || (iterOuterLoop >= maxIterOuterLoop); //|| (quadModelNbEval >= _quadModelMaxEval);
//         if (outerFailure)
//         {
//             std::cout << "Early stop: |d|=" << distXOuterLoop << " inner? " << innerFailure << " unbounded? " << unbounded_subpb << std::endl;
//         }
   ...
   )
{
    ...
    double ngproj = check_optimality_bounds(XS, GradPk, lvar, uvar, dualFeas); // TODO: one should use the Lagrangian gradient and not the augmented Lagrangian
    ...
    const bool verbose)
{
    ...
    // TODO: fine-tune these 2 in function of omega, 1E-7, 1E-15 and norm of Hessian.
    ...
    while (!innerFailure && !innerSuccess)
    {
        ...
        if (pred > 0)
        {
            // TODO the predictor could be superior to 0
...
//    while (!innerFailure && !innerSuccess)
//    {
//        // Compute the normal step (vx, vs), i.e. solve:
...
//        //      |         -mu e                               |
//        //      -                                            -
//        //  Sigma = S^-1 Y with Y the estimated Lagrange multipliers
...
//        SGTELIB::Matrix::inplace_product(r, W, vxs);
//        x0PCG.fill(0.0);
//        const double tol_PCG = std::min(tol_mu, min_tol_PCG);
//        const bool success_PCG = projected_conjugate_gradient(p, W, r, Q, qc, Delta, x0PCG, tol_PCG,verbosePCG);
//
//        if (!success_PCG)
//        {
...
//    }
    ...
    // TODO : normally, one should retrieve the former direction if the feasibility has not be reached.
    if (max_iter_reached)
    {
