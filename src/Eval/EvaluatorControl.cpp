...
NOMAD::SuccessType NOMAD::EvaluatorControl::run()
{
    ...
    // ChT. For now, the block contains eval point from the mainThread and the same evaluator
    // ChT. Maybe It could be points having only the same type of evaluator.
    bool popWorks = true;
...
bool NOMAD::EvaluatorControl::evalBlock(NOMAD::BlockForEval& blockForEval)
{
    ...
    else
    {
        if (block.size() != 0 )
        {
            getMainThreadInfo(mainThreadNum).incCurrentlyRunning(block.size()); // ChT Should be put in evalBlockOfPoints
