                          ...
                          bool &countEval) const
{
    ...
    auto compute_p = [&](int cat) -> double {
        switch (cat)
        {
        case 0: return 0.75;  // Case A
        case 1: return 1.5;   // Case B
        case 2: return 2.25;  // Case C
        case 3: return 3.0;   // Case D
        case 4: return 3.75;  // Case E
        default: return 0.0;
        }
    };

    auto compute_s = [&](int cat) -> double {
        switch (cat)
        {
        case 0: return -1.25; // Case A
        case 1: return -0.5;  // Case B
        case 2: return 0.25;  // Case C
        case 3: return 1.0;   // Case D
        case 4: return 1.75;  // Case E
        default: return 0.0;
        }
    };

    // Compute objective function
    double term1 = std::pow(std::pow(x_con[0], 2) + x_con[1] - 6 - x_int[0], 2);
    double term2 = std::pow(x_con[0] + std::pow(x_con[1], 2) - 2 - x_int[1], 2);
    double f = term1 + term2;

    // Translation to avoid min at zero
    f = f+10;

    // Compute constraints
    double p_value = compute_p(x_cat[0]);
    double s_value = compute_s(x_cat[1]);

    double g1 = -p_value * x_con[0] - x_con[1] + 0.5;
    double g2 = 3 * x_con[0] - s_value * x_con[1] + 1;
...
void initAllParams( std::shared_ptr<NOMAD::AllParameters> allParams)
{
    ...
    auto lb = NOMAD::ArrayOfDouble(N, -5);
    auto ub = NOMAD::ArrayOfDouble(N, 5);
    ...
    allParams->setAttributeValue("STATS_FILE", NOMAD::ArrayOfString("himmelblau_constrained_gmads.txt bbe sol obj cons_h"));
