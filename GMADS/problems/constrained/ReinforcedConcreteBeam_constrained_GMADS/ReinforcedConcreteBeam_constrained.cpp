...
bool My_Evaluator::eval_x(NOMAD::EvalPoint& x,
    ...
    int x_cat2 = static_cast<int>(x[1].todouble()); // Second categorical variable
    int x_int1 = static_cast<int>(x[2].todouble()); // Integer variable
    double x_con1 = x[3].todouble();                // First continuous variable
    double x_con2 = x[4].todouble();                // Second continuous variable

    // Define the table for p(x_cat1, x_cat2)
    const double p_table[5][5] = {
        {0.2, 0.52, 0.83, 1.13, 1.45},
        {0.27, 0.58, 0.87, 1.19, 1.49},
        {0.33, 0.63, 0.91, 1.24, 1.54},
        {0.38, 0.68, 0.96, 1.3,  1.62},
        {0.42, 0.73, 1.01, 1.35, 1.66}};

    // Compute p(x_cat1, x_cat2)
    double p_val = p_table[x_cat2][x_cat1];

    // Objective function
    double f = 29.4 * p_val + 0.6 * x_int1 * (x_con1 + x_con2);

    // Constraints
    double g1 = 5 * (x_con1 + x_con2) - 4 * x_int1 + 2 * p_val - 2.5;
    double g2 = 2.5 * std::pow(p_val, 2) + 25 * x_int1 - p_val * x_int1 * (x_con1 + x_con2) - 2.5;
...
void initAllParams( std::shared_ptr<NOMAD::AllParameters> allParams)
{
    ...
    //NOMAD::VariableGroup vg1 = {2, 3,4}; // quantitative variables
    ...
    allParams->setAttributeValue("STATS_FILE", NOMAD::ArrayOfString("reinforcedconcretebeam_constrained_gmads.txt bbe sol obj cons_h"));
