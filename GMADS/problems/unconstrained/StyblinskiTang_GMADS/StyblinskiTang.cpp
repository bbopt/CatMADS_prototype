                          ...
                          bool &countEval) const
{
    ...
    if (x.size() != N)
    {
        throw NOMAD::Exception(__FILE__, __LINE__, "Dimension mismatch: Ensure the number of variables matches N.");
    }

    // Extract the single categorical variable
    int x_cat = static_cast<int>(x[0].todouble()); 

    // Extract integer variables that follows the categorical variable
    std::vector<double> x_int(Nint);
    for (int i = 0; i < Nint; ++i)
    {
        x_int[i] = x[Ncat + i].todouble(); 
    }
    ...
    {
        x_con[i] = x[Ncat + Nint + i].todouble(); // Continuous variables follow integers
    }

    // Initialize the function value
    double f = 0.0;

    // Helper function to compute p(x_cat, x_con)
    auto compute_p = [&](int x_cat, double x_con) -> double {
        switch (x_cat)
        {
        case 0: // "absolute"
            return std::abs(x_con - 1);
        case 1: // "quad"
            return (std::pow(x_con, 2) + 1) / 2.0;
        case 2: // "logsum"
            return std::exp(std::abs(x_con + 1)) - 1;
        case 3: // "hyperbol"
            return std::pow(x_con, 2) / (1 + std::abs(x_con));
        case 4: // "invgauss"
            return 1 - std::exp(-std::pow(x_con, 2));
        default:
            throw NOMAD::Exception(__FILE__, __LINE__, "Invalid categorical value.");
        }
    };

    // Compute the Styblinski-Tang function with Ncon=Nint
    for (int i = 0; i < Ncon; ++i)
    {
        double x_con_i = x_con[i];
        double x_int_i = x_int[i];
        double p_val = compute_p(x_cat, x_con_i);

        f += 0.5 * (std::pow(x_con_i, 4) - 16 * std::pow(x_con_i, 2) + 5 * x_int_i + 8 * p_val);
    }
...
void initAllParams(std::shared_ptr<NOMAD::AllParameters> allParams)
{
    ...
    allParams->setAttributeValue("STATS_FILE", NOMAD::ArrayOfString("styblinskitang_gmads.txt bbe sol obj cons_h"));
