                          ...
                          bool &countEval) const
{
    ...
    // Add the absolute value of the first integer variable
    f += std::abs(x_int[0]);

    // Compute the penalty term p(x_cat2, x_con)
    double penalty = 0.0;
    double sum_con = 0.0;
    ...
    if (x_cat2 == 0) // "A"
    {
        penalty = (1.0 / Ncon) * 1.1 * std::max(0.0, sum_con);
    }
    else if (x_cat2 == 1) // "B"
    {
        penalty = (1.0 / Ncon) * (-0.9) * std::min(0.0, sum_con);
    }
    else if (x_cat2 == 2) // "C"
    {
        for (int i = 0; i < Ncon; ++i)
        {
            penalty += std::abs(x_con[i]);
        }
        penalty /= Ncon;
    }
    ...
    // Add the Rosenbrock component
    if (x_cat1 == 0) // "smooth"
    {
        for (int i = 0; i < Ncon - 1; ++i)
        {
            f += 100 * std::pow((x_con[i + 1] - std::pow(x_con[i], 2)), 2); 
            f += std::pow(x_con[i] - 1, 2);
        }
    }
    else if (x_cat1 == 1) // "nonsmooth"
    {
        for (int i = 0; i < Ncon - 1; ++i)
        {
            f += 100 * std::abs(x_con[i + 1] - std::pow(x_con[i], 2));
            f += 5 * std::abs(x_con[i] - 1); 
        }
    }
...
void initAllParams(std::shared_ptr<NOMAD::AllParameters> allParams)
{
    ...
    allParams->setAttributeValue("STATS_FILE", NOMAD::ArrayOfString("rosenbrockmixed_gmads.txt bbe sol obj cons_h"));
