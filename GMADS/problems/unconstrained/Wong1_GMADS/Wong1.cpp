                          ...
                          bool &countEval) const
{
    ...
    // Ensure the number of continuous and integer variables matches expectations
    if (Ncon != 4 || Nint != 3)
    {
        throw NOMAD::Exception(__FILE__, __LINE__, "Expected Ncon = 4 and Nint = 3.");
    }

    // Helper function for p(...)
    auto compute_p = [&](const std::vector<int> &x_int, const std::vector<double> &x_con) -> double {
        return std::pow(x_con[0] - 10, 2) +
               5 * std::pow(x_int[0] - 12, 2) +
               std::pow(x_con[1], 4) +
               3 * std::pow(x_int[1] - 11, 2) +
               10 * std::pow(x_con[2], 6) +
               7 * std::pow(x_int[2], 2) +
               std::pow(x_con[3], 4) -
               4 * x_int[2] * x_con[3] -
               10 * x_int[2] -
    ...
    // Compute p(...)
    double p_val = compute_p(x_int, x_con);

    // Initialize the function value with p(...)
    double f = p_val;

    // Add contributions based on the categorical variable
    switch (x_cat[0])
    {
    case 1: // "B"
        f += 10 * (2 * std::pow(x_con[0], 2) + 3 * std::pow(x_int[0], 4) + x_con[1] +
                   std::pow(x_int[1], 2) + 5 * x_con[2] - 127);
        break;
    case 2: // "C"
        f += 10 * (7 * x_con[0] + 3 * x_int[0] + 10 * std::pow(x_con[1], 3) +
                   x_int[1] - x_con[2] - 282);
        break;
    case 3: // "D"
        f += 10 * (23 * x_con[0] + std::pow(x_int[0], 2) + 6 * x_int[2] -
                   8 * x_con[3] - 196);
        break;
    case 4: // "E"
        f += 10 * (4 * std::pow(x_con[0], 2) + x_int[0] - 3 * x_con[0] * x_int[0] +
                   2 * std::pow(x_con[1], 2) + 5 * x_int[2] - 11 * x_con[3]);
        break;
    default: // "A" or other
        // No additional contributions for "A"
        break;
    }
...
void initAllParams(std::shared_ptr<NOMAD::AllParameters> allParams)
{
    ...
    //NOMAD::VariableGroup vg1 = {1,2,3, 4,5,6,7}; // quantitative variables
    ...
    allParams->setAttributeValue("STATS_FILE", NOMAD::ArrayOfString("wong1_gmads.txt bbe sol obj cons_h"));
