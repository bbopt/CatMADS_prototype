                          ...
                          bool &countEval) const
{
    ...
    double x_con1 = x[Ncat].todouble();
    double x_con2 = x[Ncat + 1].todouble();

    // Determine x_3^{continuous} and x_4^{continuous} based on categorical variables
    double x_con3 = 0.0, x_con4 = 0.0;
    if (x_cat1 == 0)
    {
        if (x_cat2 == 0)
        {
    ...
    {
        throw NOMAD::Exception(__FILE__, __LINE__, "Invalid categorical variable value for x_cat1 or x_cat2.");
    }

    // Compute the p(...) function
    double p = 53.3108 +
               0.184901 * x_con1 -
               5.02914 * std::pow(x_con1, 3) * 1e-6 +
               7.72522 * std::pow(x_con1, 4) * 1e-8 -
               0.0870775 * x_con2 -
               0.106959 * x_con3 +
               7.98772 * std::pow(x_con3, 3) * 1e-6 +
               0.00242482 * x_con4 +
               1.32851 * std::pow(x_con4, 3) * 1e-6 -
               0.00146393 * x_con1 * x_con2 -
               0.00301588 * x_con1 * x_con3 -
               0.00272291 * x_con1 * x_con4 +
               0.0017004 * x_con2 * x_con3 +
               0.0038428 * x_con2 * x_con4 -
               0.000198969 * x_con3 * x_con4 +
               1.86025 * x_con1 * x_con2 * x_con3 * 1e-5 -
               1.88719 * x_con1 * x_con2 * x_con4 * 1e-6 +
               2.50923 * x_con1 * x_con3 * x_con4 * 1e-5 -
               5.62199 * x_con2 * x_con3 * x_con4 * 1e-5;
...
void initAllParams(std::shared_ptr<NOMAD::AllParameters> allParams)
{
    ...
    allParams->setAttributeValue("STATS_FILE", NOMAD::ArrayOfString("goldsteinprice1_gmads.txt bbe sol obj cons_h"));
