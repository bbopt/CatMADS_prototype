                          ...
                          bool &countEval) const
{
    ...
    // Ensure the number of continuous variables is exactly 4
    if (Ncon != 4)
    {
        throw NOMAD::Exception(__FILE__, __LINE__, "The number of continuous variables must be 4.");
    }

    // Helper function for p(...)
    auto compute_p = [&](int x_int, const std::vector<double> &x_con) -> double {
        return std::pow(x_con[0], 2) + std::pow(x_con[1], 2) + 2 * std::pow(x_con[2], 2) + std::pow(x_con[3], 2) 
                    - 5 * x_con[0] - 5 * x_con[1] - 21 * x_con[2] + 7 * x_con[3] + x_int * x_con[2];
    };

    // Compute p(x_1^{int}, x^{con})
    double p_val = compute_p(x_int[0], x_con);

    // Initialize f with p(...)
    double f = p_val;

    // Add the categorical case contributions
    if (x_cat[0] == 1) // "B"
    {
        f += 10 * (std::pow(x_con[0], 2) + std::pow(x_con[1], 2) + std::pow(x_con[2], 2) +
                   std::pow(x_con[3], 2) + x_con[0] - x_con[1] + x_con[2] - x_con[3] - x_int[0] - 8);
    }
    else if (x_cat[0] == 2) // "C"
    {
        f += 10 * (std::pow(x_con[0], 2) + 2 * std::pow(x_con[1], 2) + std::pow(x_con[2], 2) +
                   2 * std::pow(x_con[3], 2) - x_con[0] - x_con[3] - 2 * x_int[0] - 10);
    }
    else if (x_cat[0] == 3) // "D"
    {
        f += 10 * (2 * std::pow(x_con[0], 2) + std::pow(x_con[1], 2) + std::pow(x_con[2], 2) +
                   2 * std::pow(x_con[3], 2) - x_con[0] - x_con[1] - 3 * x_int[0] - 5);
    }
...
void initAllParams(std::shared_ptr<NOMAD::AllParameters> allParams)
{
    ...
    //NOMAD::VariableGroup vg1 = {1, 2,3,4,5}; // quantitative variables
    ...
    allParams->setAttributeValue("STATS_FILE", NOMAD::ArrayOfString("rosensuzuki_gmads.txt bbe sol obj cons_h"));
